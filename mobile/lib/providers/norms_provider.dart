  ...
  Future<void> loadNorms(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          debugPrint('Norms API error: ${response.statusCode}');
        }
      }
    } catch (e) {
      _error = 'Ошибка сети: $e';
      if (kDebugMode) {
        debugPrint('Norms API exception: $e');
      }
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  Future<bool> addNorm(BuildContext context, Norm norm) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$_baseUrl/admin/norms'),
        headers: {
          'Authorization': 'Bearer ${authProvider.token}',
          'Content-Type': 'application/json',
        },
  ...
  Future<bool> updateNorm(BuildContext context, Norm norm) async {
    ...
    try {
      final response = await http.patch(
        Uri.parse('$_baseUrl/admin/norms/${norm.id}'),
        headers: {
          'Authorization': 'Bearer ${authProvider.token}',
          'Content-Type': 'application/json',
        },
  ...
  Future<bool> deleteNorm(BuildContext context, int normId) async {
    ...
    try {
      final response = await http.delete(
        Uri.parse('$_baseUrl/admin/norms/$normId'),
        headers: {
          'Authorization': 'Bearer ${authProvider.token}',
          'Content-Type': 'application/json',
        },
