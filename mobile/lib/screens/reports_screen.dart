...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<ReportsProvider>(
        builder: (context, reportsProvider, child) {
          final isPatient = context.watch<AuthProvider>().isPatient;
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      isPatient
                          ? 'Когда врач загрузит файл с вашим кодом пациента, отчёт появится здесь.'
                        ...
                        trailing: PopupMenuButton<String>(
                          onSelected: (value) async {
                            if (value == 'view') {
                              ...
                              context.push('/reports/${report.id}/row-bind', extra: false);
                ...
                Consumer<ReportsProvider>(
                  builder: (context, provider, _) {
                    ...
                    if (provider.shouldShowReportDetails && provider.currentReport.isNotEmpty) {
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        if (mounted) {
                          final meta = provider.reportToShow;
                          if (meta is Report) {
                            provider.loadReportAnalytics(context, meta.id);
                          }
                      ...
                      return Card(
                        ...
                        child: ExpansionTile(
                          ...
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Возраст: ${patientReport.age} лет'),
                              if (patientReport.attendingDoctorLabel != null &&
                                  patientReport.attendingDoctorLabel!.trim().isNotEmpty)
                                Padding(
                                  padding: const EdgeInsets.only(top: 4),
                                  child: Text(
                                    'Ведущий врач: ${patientReport.attendingDoctorLabel}',
                                    style: TextStyle(color: Colors.blue.shade800, fontSize: 13),
                                  ),
                                ),
                              if (patientReport.reportUploadedByLabel != null &&
                                  patientReport.reportUploadedByLabel!.trim().isNotEmpty)
                                Padding(
                                  padding: const EdgeInsets.only(top: 2),
                                  child: Text(
                                    'Отчёт загрузил: ${patientReport.reportUploadedByLabel}',
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  _expertFindingsForPatient(context, reportsProvider, patientReport, report),
                                ],
                              ),
                            ),
                          ],
  ...
  Widget _expertFindingsForPatient(BuildContext context, ReportsProvider reportsProvider,
      PatientReport pr, dynamic report) {
    final rpId = reportPatientIdFromAnalytics(reportsProvider, pr);
    if (rpId == null) {
      return const SizedBox.shrink();
    }
    final auth = context.read<AuthProvider>();
    return ExpertFindingsBlock(
      reportPatientId: rpId,
      onChatWithPatient: () => _openChat(context, auth, pr, report),
    );
  }

  Future<void> _openChat(BuildContext context, AuthProvider auth, PatientReport pr, dynamic report) async {
    final chat = context.read<ChatProvider>();
    int? tid;
    if (auth.isPatient) {
      tid = await chat.openThreadWithMyDoctor(auth,
          subject: 'Обсуждение отчёта №${report?.id ?? ''}');
    } else if (pr.linkPatientId != null) {
      tid = await chat.openThreadByPatientId(auth,
          patientId: pr.linkPatientId!,
          subject: 'Обсуждение отчёта №${report?.id ?? ''}, пациент ${pr.code}');
    } else {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Пациент не привязан к карточке — чат невозможен')),
        );
      }
      return;
    }
    ...
    if (tid != null) {
      ...
      final err = chat.threadsError ?? 'Не удалось открыть чат';
