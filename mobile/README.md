...
3. **Настройка бэкенда (Java, порт 8080):**
   
   В `lib/providers/auth_provider.dart`, `norms_provider.dart`, `reports_provider.dart` задан `_baseUrl`.
   
   - **iOS симулятор / локально:** `http://localhost:8080` (уже стоит).
   - **Android эмулятор:** замените на `http://10.0.2.2:8080` (доступ с эмулятора к ПК).
   - **Реальное устройство:** укажите IP компьютера, например `http://192.168.1.100:8080`.
   
   Бэкенд должен быть запущен (Docker или IDE) перед запуском приложения.
